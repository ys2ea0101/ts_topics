"""
Code directly copied from: https://github.com/philipperemy/keras-tcn/blob/master/tcn/tcn.py
but modified to fit the same output as direct model
"""
...
class TCN(Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.padding_same_and_time_dim_unknown = (
            False  # edge case if padding='same' and time_dim = None
        )
    ...
    def build(self, input_shape):
        ...
        if self.padding == "same":
            ...
            if (
                time is not None
            ):  # if time dimension is defined. e.g. shape = (bs, 500, input_dim).
    ...
    def compute_output_shape(self, input_shape):
        ...
        if not self.return_sequences:
            ...
            batch_size = (
                batch_size.value if hasattr(batch_size, "value") else batch_size
            )
            ...
            return [
                v.value if hasattr(v, "value") else v for v in self.build_output_shape
            ]
...
def TCN_iter_pred(model, data, pred_step):
    """
    iterative prediction of TCN model,

    :param model:
    :param data: data shape:  N_batch * window_size * 1,
    :param pred_step:
    :return:
        output of shape: N_batch * pred_step
    """
    sout = model(data)
    result = np.copy(sout[:,-1:,:])
    new_input = data
    for i in range(1, pred_step):
        new_input = np.concatenate((new_input[:, 1:, :], sout[:,-1:,:]), axis=1)
        sout = model(new_input)
        result = np.concatenate((result, sout[:,-1:,:]), axis=1)
